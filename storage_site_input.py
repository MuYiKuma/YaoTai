...
class StorageSiteInput:
    ...
    # Site / System Basics
    # =========================
    contract_capacity_kw: float = field(
        metadata={"unit": "kW", "description": "契約容量"}
    )
    power_kw: float = field(
        metadata={"unit": "kW", "description": "儲能PCS額定功率"}
    )
    capacity_kwh: float = field(
        metadata={"unit": "kWh", "description": "儲能電池額定容量"}
    )
    ...
    dod: float = field(
        default=0.95,
        metadata={"unit": "ratio", "description": "放電深度（0~1）"}
    )
    ...
    soh: float = field(
        default=1.0,
        metadata={"unit": "ratio", "description": "當前年份SOH（0~1）"}
    )
    ...
    dr_enabled: bool = field(
        default=False,
        metadata={"description": "是否啟用需量反應收益"}
    )
    ...
    # =========================
    sr_enabled: bool = field(
        default=False,
        metadata={"description": "是否啟用即時備轉收益"}
    )
    ...
    analysis_years: int = field(
        default=1,
        metadata={"unit": "years", "description": "財務分析年限"}
    )
    ...
    discount_rate: float = field(
        default=0.0,
        metadata={"unit": "ratio", "description": "折現率（0~1）"}
    )

    # =========================
    # Optional bookkeeping / trace
    # =========================
    source_sheet_name: Optional[str] = field(
        default=None,
        metadata={"description": "來源sheet名稱（供trace/debug使用）"}
    )
    source_case_name: Optional[str] = field(
        default=None,
        metadata={"description": "案場名稱或案例識別"}
    )
